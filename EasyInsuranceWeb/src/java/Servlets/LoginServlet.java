...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        TypedQuery<Customers> query
                = em.createNamedQuery("Customers.findByLogin", Customers.class).setParameter("email", request.getParameter("email")).setParameter("password", request.getParameter("password"));
        List<Customers> list = query.getResultList();
        if (list.size() == 1) {
            Customers user = list.get(0);
            ...
            Cookie lastName = new Cookie("last_name",
                    user.getSurname());
            Cookie username = new Cookie("email",
                    user.getEmail());
